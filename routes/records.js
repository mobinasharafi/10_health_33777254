...
router.get('/usr/455/records', async (req, res) => {
    try {
...
router.get('/usr/455/records/add', (req, res) => {
    if (!req.session.userId) {
        return res.redirect('/usr/455/login');
    }
...
router.post('/usr/455/records/add', async (req, res) => {
    try {
        if (!req.session.userId) {
            return res.redirect('/usr/455/login');
        }
...
router.get('/usr/455/records/edit/:id', async (req, res) => {
    try {
        if (!req.session.userId) {
            return res.redirect('/usr/455/login');
        }
...
router.put('/usr/455/records/edit/:id', async (req, res) => {
    try {
        if (!req.session.userId) {
            return res.redirect('/usr/455/login');
        }
...
router.get('/usr/455/records/search', async (req, res) => {
    try {
        if (!req.session.userId) {
            return res.redirect('/usr/455/login');
        }
...
// Delete a record
router.delete('/usr/455/records/delete/:id', async (req, res) => {
    try {
        if (!req.session.userId) {
            return res.redirect('/usr/455/login');
        }
...
router.get('/usr/455/records/suggest', async (req, res) => {
